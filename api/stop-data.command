#!/bin/bash
# macOS stopper for data pipeline: stops Watchdog + Embedder (+ API optional)
# Usage:
#   ./stop-data.command [--and-api]
...
AND_API=0
if [[ "${1:-}" == "--and-api" ]]; then AND_API=1; fi
...
main() {
  ...
  if [[ ${AND_API} -eq 1 ]]; then
    stop_by_pidfile "${API_PID_FILE}" "RAG API"
    stop_by_port 5001 "RAG API"
  fi
  log "Data pipeline processes have been stopped."
}
